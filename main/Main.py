...
from raycaster.physics import *
from raycaster.observer import Scene, Observer
from raycaster.objects.surface import Plane, Cone, Cylinder, Sphere
...
# Cenário
observer = Observer.A(Point(5, 5, 30), Point(5, 1, 27), resolution, size=(7, 7))
scene = Scene()
...
scene.add_obj(Cone(Point(10, 4, 10), Point(10, 7, 10), 1.5), "tree_2_leaves", colors["red"])

# Executar o raycast e desenhar na tela.
for y_index in range(resolution[1]):
    for x_index in range(resolution[0]):
        line = observer.shoot(x_index, y_index) # lista de pixels
        min_coef = None
        min_obj = None
